...
def query_longdo(word):
    ...
    try:
        ...
        return {
            "thai": word if _is_thai(word) else None,
            "english": None if _is_thai(word) else word,
            "phonetic_ipa": None,
            "part_of_speech": None,
            "translations": [],
            "error": str(e)
...
def _parse_entries(html, query_word, final_ipa):
    ...
    dict_pattern = r'<b>(.*?)</b>(?:<br/>)?<table class=\'result-table\'>(.*EN-TH.*?)</table>'
    ...
    for dict_name, table_content in dict_matches:
        ...
        if 'ระวัง' in dict_name or 'unapproved' in dict_name.lower() or not 'TH-EN' in dict_name:
            continue
